...
class KVCache(torch.nn.Module):
    def __init__(self, is_k=True):
        super(KVCache, self).__init__()
        self._cache = None
        self.inp_seq_len = -1
        self.is_k_states = is_k
        self.compress_cache = False

    @property
    def cache(self):
        if isinstance(self._cache, CompressedUnion):
            return self._cache.decompress()
        else:
            return self._cache
    ...
    def cache(self, cache):
        if isinstance(self._cache, CompressedUnion):
            self._cache.set_cache(cache)
            self._cache.compress()
        else:
            self._cache = cache
    ...
    def allocate(self, inp_seq_len, dtype, device, shape):
        if self._cache is None:
            ...
            self._cache = torch.zeros(shape, dtype=dtype, device=device)
            config_path = os.getenv("KV_QUANT_CONFIG", None)
            if config_path is not None:
                ...
                union = CompressedUnion(self.quantize_bit, self.compress_mode, self.group_size)
                union.set_cache(self._cache)
                union.compress()
                self._cache = union
        elif self._cache.shape != shape:
            self.inp_seq_len = inp_seq_len
            self._cache = torch.zeros(shape, dtype=dtype, device=device)
        else:
            assert (
    ...
    def update(self, prev, cur, dim, idx, inp_seq_len):
        if isinstance(prev, CompressedUnion):
            prev = prev.decompress()
        orig_cur = cur
        if prev.shape == cur.shape:
            prev.copy_(cur)
            if isinstance(self._cache, CompressedUnion):
                self._cache.set_cache(prev)
                self._cache.compress()
            return orig_cur
    ...
    def forward(self, cur, dim, idx):
        return self.update(self._cache, cur, dim, idx, self.inp_seq_len)
